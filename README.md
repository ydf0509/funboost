...
### 3.1.3控频功能证明，使用内网连接broker，设置每秒运行100次需要消耗任意随机时长的函数，能够做到持续精确控频，频率误差小。如果设置每秒精确运行超过5000次以上的固定频率，前提是cpu够强。
...
```
 ...
 #### 5.4 各种中间件的优劣？
   ...
   答： python内置Queue对象作为当前解释器下的消息队列，
       ...
       sqlalchemy可确认消费方式，作为消息队列，
       优势： 真分布式，一次性支持了5种关系型数据库每个队列由一张表来模拟，每个任物是表中的一行；比直接弹出元素的队列，方便统计消费情况。
