...
```
 ...
 #### 5.4 各种中间件的优劣？
   ...
   答： python内置Queue对象作为当前解释器下的消息队列，
       ...
       kafka 作为消息队列：
       优势：性能好吞吐量大，中间件扩展好，实测外网发布10000条消息到中间件，kafka比rabbitmq快。kafka的分组消费设计得很好，
            还可以随时重置偏移量，不需要重复发布旧消息。
       劣势：和传统mq设计很大的不同，使用偏移量来作为断点接续依据，不是原子性的。高并发数量时候，
            主题的分区如果设置少，确认消费实现难。要求可靠性 一致性高还是用rabbitmq好。
            
       nsq 作为消息队列：
       优势：部署安装很简单，比rabittmq和kafka安装简单。性能不错。
       劣势：用户很少，比rabbitmq用户少了几百倍，导致资料也很少。
