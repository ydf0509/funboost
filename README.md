...
### 6.13.3 框架的asyncio并发模式的目前还可以优化的点。
```
异步代码需要一旦异步处处异步，比如你用了aiohttp 但是异步函数里面又写了个time.sleep(1)，那就完蛋了，
无论怎么优化协程异步调度，最多1秒只能执行一次函数。

框架里面做确认消费是在函数运行完成后执行，这个动作是和函数处在一个线程/协程运行。
当使用async并发模式时候，由于框架目前操作的所有消息中间件都是使用的同步包，没有使用异步的包例如 aioredis  aiorabbitmq aiomysql这种。
所以每个任务在做确认消费时候会阻塞几百微妙。但是好在这种动作不需要消耗大量时间，所以对整体运行效率无伤大雅。
相当于在异步函数里面写了个time.sleep(0.0005) 虽然比 await asyncio.sleep(0.0005)会造成独占阻塞一会儿，但阻塞的时间不长就无伤大雅。
目前celery在对无io 无cpu的 print hello函数进行调度时候一秒钟单核只能调度300次。所以此框架的asyncio模式做确认消费阻塞几百微妙不算什么严重事情还。

如果操作所有中间件都改成异步操作的包，那么需要花很大精力大改特该的修改，带来的效益不是很高，所以目前不打算这么做，
但这是日后可以对某几个重要典型的中间件值得优化的点。
