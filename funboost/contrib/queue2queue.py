...
def consume_and_push_to_another_queue(source_queue_name: str, source_broker_kind: int,
    ...
    if exit_script_when_finish:
        print(f'消息转移完成，结束脚本,累计从 {source_queue_name} 转移消息到 {target_queue_name} 队列 个数是 {msg_cnt}')
        os._exit(888)  # 结束脚本


if __name__ == '__main__':
    consume_and_push_to_another_queue('test_queue77h3_dlx2', 0, 'test_queue77h3_dlx3', 0, log_level_int=logging.INFO, exit_script_when_finish=True)
