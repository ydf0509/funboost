...
from funboost.assist import user_custom_broker_register
...
def get_consumer(*args, broker_kind: int = None, **kwargs) -> AbstractConsumer:
    ...
    if broker_kind in user_custom_broker_register.broker_kind__regist_fun_map:
        user_custom_broker_register.broker_kind__regist_fun_map[broker_kind]()  # 动态注册中间件到框架是为了延迟导入，用户没安装不需要的第三方包不报错。
