...
class RedisStreamConsumer(AbstractConsumer, RedisMixin):
    ...
    def _shedual_task(self):
        try:
            self.redis_db_frame_version3.xgroup_create(self._queue_name, 'distributed_frame_group', id=0, mkstream=True)
            ...
            if lock.has_aquire_lock:
                ...
                for xinfo_item in xinfo_consumers:
                    ...
                    if xinfo_item['idle'] > 7 * 24 * 3600 * 1000 and xinfo_item['pending'] == 0:
                        self.redis_db_frame_version3.xgroup_delconsumer(self._queue_name, 'distributed_frame_group', xinfo_item['name'])
                    if xinfo_item['name'] not in current_queue_hearbeat_ids and xinfo_item['pending'] > 0:  # 说明这个消费者掉线断开或者关闭了。
                        ...
                        if pending_msg_list:
                            ...
                            xclaim_task_list = self.redis_db_frame_version3.xclaim(self._queue_name, 'distributed_frame_group', self.consumer_identification, force=True,
                                                                                   min_idle_time=0 * 1000, message_ids=[task_item['message_id'] for task_item in pending_msg_list])
                            if xclaim_task_list:
                                self.logger.warning(f' {self._queue_name}  的分组 distributed_frame_group 的消费者 {self.consumer_identification} 夺取 断开的消费者 {xinfo_item["name"]}'
