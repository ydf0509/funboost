...
class RedisStreamConsumer(AbstractConsumer, RedisMixin):
    ...
    def _shedual_task(self):
        ...
        except redis3.exceptions.ResponseError:
            pass  # BUSYGROUP Consumer Group name already exists  不能重复创建消费者组。
    ...
    def _requeue(self, kw):
        ...
        self.redis_db_frame_version3.xclaim(self._queue_name,
        ...
        with decorators.RedisDistributedLockContextManager(self.redis_db_frame, lock_key, ) as lock:
            if lock.has_aquire_lock:
                ...
                for xinfo_item in xinfo_consumers:
                    if xinfo_item['name'] not in current_queue_hearbeat_ids:
                        ...
                        if pending_msg_list:
                            self.logger.warning(f'向 {self._queue_name} 中 转移已断开掉线的任务 {pending_msg_list}')
                            self.redis_db_frame_version3.xclaim(self._queue_name, 'distributed_frame_group', self.consumer_identification,
