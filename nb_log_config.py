...
FORMATTER_DICT = {
    ...
    9: logging.Formatter(
        '%(asctime)s - %(name)s [p%(process)d_t%(thread)d] "%(pathname)s:%(lineno)d" - %(funcName)s - %(levelname)s - %(message)s',
        "%Y-%m-%d %H:%M:%S"),  # 对5改进，带进程和线程显示的日志模板。
    10: logging.Formatter(
        '%(asctime)s - %(name)s [p%(process)d_t%(thread)d] "%(filename)s:%(lineno)d" - %(levelname)s - %(message)s', "%Y-%m-%d %H:%M:%S"),  # 对7改进，带进程和线程显示的日志模板。
}
