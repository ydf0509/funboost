...
consumer_add = get_consumer('queue_test569', consuming_function=add, concurrent_num=5000, max_retry_times=2,
                            ...
                            broker_kind=10, concurrent_mode=1, )  # 通过设置broker_kind，一键切换中间件为rabbitmq或redis等9种中间件或包。

consumer_sub = get_consumer('queue_test84', consuming_function=sub, concurrent_num=2000, qps=108, log_level=10, logger_prefix='xxxxx平台消费',
                            is_print_detail_exception=True,
                            broker_kind=10, concurrent_mode=1)  # 通过设置
