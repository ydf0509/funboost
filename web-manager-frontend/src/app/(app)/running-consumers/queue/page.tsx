...
export default function ConsumersByQueuePage() {
  ...
  const loadPartitions = useCallback(async () => {
    try {
      ...
      let url = "/running_consumer/hearbeat_info_partion_by_queue_name";
  ...
  const loadConsumers = useCallback(async () => {
    ...
    try {
      ...
      // 构建 API URL，添加 project_id 参数
      let url = `/running_consumer/hearbeat_info_by_queue_name?queue_name=${encodeURIComponent(queryValue)}`;
      if (currentProject?.id) {
        url += `&project_id=${currentProject.id}`;
      }
  ...
  return (
    <RequirePermission permissions={["queue:read"]} projectLevel="read">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard label="活跃队列" value={partitions.length} helper="有消费者" />
        <StatCard label="消费者" value={consumers.length} helper="当前运行" />
        <StatCard label="范围" value={selected} helper="已选队列" />
      </div>

      <Card>
        <SectionHeader title="筛选" subtitle="选择队列进行聚焦。" />
